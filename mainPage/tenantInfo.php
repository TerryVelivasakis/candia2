...
<select onchange="checkdata()" class=form-select id="tenantSelect">
  <?php
  $sql = "SELECT * from executiveLease WHERE Property =".$currentProperty;
  $result = $db->query($sql);
$x=0;
  while($row = $result->fetch_assoc()) {
  echo '<option value = '.$x.$row[''];
...
<table class="table table-sm">
  ...
  <tr><th>Email</th><td id="contactTableEmail"> Tenant 1</td></tr>
...
console.log($("#tenantSelect").find(':selected').data('tenantname'));
